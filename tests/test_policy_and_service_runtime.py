...
def test_phase_service_unavailable_skips_unknown_faq_action_for_validator_fallback(monkeypatch):
    service = ChatService()
    context = ConversationContext(
...
def test_table_booking_capability_check_blocks_room_stay_booking_language(monkeypatch):
    service = ChatService()
    intent_result = IntentResult(
        intent=IntentType.TABLE_BOOKING,
        confidence=0.85,
    ...
    assert check.allowed is False
    assert "room-stay booking" in check.reason.lower()


def test_table_booking_capability_check_blocks_room_stay_lookup_language(monkeypatch):
    service = ChatService()
    intent_result = IntentResult(
        intent=IntentType.TABLE_BOOKING,
        confidence=0.85,
